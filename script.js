const SCAVENGER_HUNT_QUESTION = "What is 2FA";
const ANIM_SPEED = 10;
...
function displayConsoleOutput(textArray, elementId, delay = 500) {
    ...
    function hideAllExceptNoiseAndOverlay() {
        setTimeout(function() {
...
function scrambleText(elementId, duration, speed) {
    ...
    function updateText() {
        if (currentIteration < iterations) {
            ...
            console.log('running', element, scramble)
            element.textContent = scramble;
            currentIteration++;
            setTimeout(updateText, speed);
        } else {
            // set the final text here
            element.textContent = "What is 2FA?";
        }
    }
